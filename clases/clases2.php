...
define ('CONEXION', $_SERVER["DOCUMENT_ROOT"].'/conexion/');  // Innovate
include (CONEXION.'conexion.php');

//define ('RAIZ', '/tests/mensajeria'); // leuss
//define ('RAIZ', '/mensajeria');  // localhost
define ('RAIZ', "");  // Innovate
/*******************************************/
...
function obtenerFechayHora($fechauhora,$formatofecha='d/m/Y',$formatohora='g:i:a'){
	...
	$hora=date('g')-1; // ojo algunos sistemas aceptan la hora con -1
	$min=date('i');
	$ampm=date('a');
	$hora=$hora.":".$min.$ampm;
...
function obtenerDatosCiudad($nombreciudaddestino)
{
	$nombreciudaddestino = strtolower( trim($nombreciudaddestino) );
	$SQL = "SELECT idciudad, nombre_ciudad, departamento_iddepartamento, pais_ciudad from ciudad WHERE LOWER( TRIM(nombre_ciudad)) = \"$nombreciudaddestino\"";
...
<div id="menu" style="position:absolute; top:10px; left:500px ">
 ...
 <li>
  ...
  <li><a class="MenuBarItemSubmenu" href="#"  style="text-decoration:none">Unitario</a>
      <ul>
        ...
        <li><a  href="<?=RAIZ?>/gestion/ordendeservicio/addosunitario.php">Digitar Gu&iacute;a</a>
        </li>
      </ul>
  </li>
...
$this->idmanifiesto = 1;
$SQL = sprintf("INSERT INTO manifiesto (idmanifiesto,num_manifiesto,tercero_idmensajero_recibe,tercero_idmensajero_entrega,sucursal_idsucursal,plazo_entrega_manifiesto)
values('%s','%s','%s','%s','%s','%s')
",$this->idmanifiesto,$this->num_manifiesto,$this->tercero_idmensajero_recibe,$this->tercero_idmensajero_entrega,$this->sucursal_idsucursal,$this->plazo_entrega_manifiesto);
if ($conn->ejecutar($SQL))
return true;
else
$this->idmanifiesto="";
...
function modificar(){
global $conn;
$SQL = sprintf("UPDATE manifiesto SET idmanifiesto='%s',num_manifiesto='%s',tercero_idmensajero_recibe='%s',tercero_idmensajero_entrega='%s',sucursal_idsucursal='%s',plazo_entrega_manifiesto='%s' WHERE idmanifiesto=%d "
,$this->idmanifiesto,$this->num_manifiesto,$this->tercero_idmensajero_recibe,$this->tercero_idmensajero_entrega,$this->sucursal_idsucursal,$this->plazo_entrega_manifiesto,$this->idmanifiesto);
if ($conn->ejecutar($SQL))
return true;
}
...
function modificar2($stringset,$idos){
global $conn;
$SQL = "UPDATE manifiesto SET $stringset  WHERE idmanifiesto=$idos";
if ($conn->ejecutar($SQL))
return true;
}
...
$this->idmovimiento = 1;
$SQL = sprintf("INSERT INTO movimiento (idmovimiento,estado_idestado,tercero_idusuario,fecha_movimiento,hora_movimiento,recogida_idrecogida,orden_servicio_idorden_servicio,manifiesto_idmanifiesto,guia_idguia,sucursal_idsucursal,asignacion_guias_idasignacion_guias,tarifa_idtarifa)
values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')
",$this->idmovimiento,$this->estado_idestado,$this->tercero_idusuario,$this->fecha_movimiento,$this->hora_movimiento,$this->recogida_idrecogida,$this->orden_servicio_idorden_servicio,$this->manifiesto_idmanifiesto,$this->guia_idguia,$this->sucursal_idsucursal,$this->asignacion_guias_idasignacion_guias,$this->tarifa_idtarifa);
if ($conn->ejecutar($SQL))
return true;
else
$this->idmovimiento="";
...
$SQL = sprintf("INSERT INTO asignacion_guias (idasignacion_guias,sucursal_idsucursal,tercero_idtercero,fecha_asignacion,hora_asignacion,inicial_asignacion,cantidad_asignacion,saldo_asignnacion,estado_asignacion,observaciones_asignacion)
values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')
",$this->idasignacion_guias,$this->sucursal_idsucursal,$this->tercero_idtercero,$this->fecha_asignacion,$this->hora_asignacion,$this->inicial_asignacion,$this->cantidad_asignacion,$this->saldo_asignnacion,$this->estado_asignacion,$this->observaciones_asignacion);
if ($conn->ejecutar($SQL))
return true;
else
