...
//            return;
           ...
           while ($camposguia[$j])
           {
               ...
               $datosciudad = $operaciones->obtenerDatosCiudad($camposguia[$j]["ciudaddestino"], $camposguia[$j]["municipiodestino"]);

               if(empty($datosciudad) || empty($idTipoProducto))
               {
                   die("Error en la fila".' '.($j+1));
               }
