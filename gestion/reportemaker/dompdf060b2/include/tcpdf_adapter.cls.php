...
class TCPDF_Adapter implements Canvas {
	...
	function __construct($paper = "letter", $orientation = "portrait") {
		dompdf_debug("trace", "($paper, $orientation)");
		if (is_array($paper)) {
			$size = $paper;
		}
		else {
		...
		dompdf_debug("trace", "size = " . json_encode($size));
	...
	function add_info($label, $value) {
		dompdf_debug("trace", "($label, $value)");
	...
	function add_object($object, $where = 'all') {
		dompdf_debug("trace", "($object, $where)");
