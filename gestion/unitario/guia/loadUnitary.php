...
//      $idremitente = $fil["tercero_idcliente"];
  ...
  if ($error == UPLOAD_ERR_OK)
  {
    ...
    for ($i = 0; $i < $tamanio; $i++)
    {
      list($idremitente, $valorDeclarado,$ciudaddestino, $municipiodestino, $cedula, $nombre, $apellido, $direccion, $telefono, $celular, $observacion) = explode(";", $lineas[$i]);
      if ($idremitente == "" || $ciudaddestino == "" || $municipiodestino == '' || $nombre == '' || $direccion == '')
      {
      ...
      $camposguia[$i]["vrDeclarado"] = $valorDeclarado;
      $idremitente = eregi_replace("[\n|\r|\n\r]", ' ', $idremitente);
      $camposguia[$i]["idRemitente"] = $idremitente;
    ...
    while ($camposguia[$j])
    {
      ...
      $camposguia[$j]["idRemitente"] = getIdTercero($camposguia[$j]["idRemitente"]);
      if(!$camposguia[$j]["idRemitente"])
      {
        print_r($camposguia[$j]);
        die("<br>Error al validar el id del remitente, fila: " . ' ' . ($j + 1));
      }
...
//      $idproducto = $fila['idproducto'];
      ...
      $guia->producto_idproducto = $idTipoProducto;
      ...
      $guia->owner = $camposguia[$j]["idRemitente"];
      $guia->peso_guia = 10;
      $guia->ciudad_idorigen = $idciudadorigen;
      $guia->tercero_idremitente = $camposguia[$i]["idRemitente"];
      $ced = $camposguia[$j]["cedula"];
      $guia->valor_declarado_guia = $camposguia[$j]["vrDeclarado"];
...
(1,'$ced','$nom','','') ON DUPLICATE KEY UPDATE iddestinatario=LAST_INSERT_ID(iddestinatario), tipo_identificacion_destinatario = 1";
      ...
      if (mysql_query($queryDesti))
      {
        ...
        $rguia = $guia->agregarUnitario($idCreator);
        if ($rguia == false)
        {
          $qtrans = "ROLLBACK;";
          $sac = $conex->ejecutar($qtrans);
  ...
  if ($rguia === true)
  {
    ...
    <script language="javascript" type="text/javascript">
      ...
      alert(mensaje);
      window.location.href = 'loadUnitary.php';
    </script>
...
?>	
      ...
      <p><b>Nota: </b>La ciudad de origen de las guias cargadas sera la misma asociada a la cuenta del usuario actual.</p>
...
?>
      ...
      <p>Esta funcion es experimental.</p>
    </div>
  </body>
</html>
...
/**
 * metodo para obtener el id de un tercero
 * @param type $cedula
 * 
 * @return int id del tercero
 */
function getIdTercero($cedula)
{
  $SQL = "SELECT * FROM tercero t WHERE t.documento_tercero = '$cedula'";
//   echo("<br>" . $SQL . "<br>");
  $res = mysql_query($SQL);
  if ($res)
  {
