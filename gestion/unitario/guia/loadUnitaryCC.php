...
//      $idremitente = $fil["tercero_idcliente"];
  ...
  if ($error == UPLOAD_ERR_OK)
  {
    ...
    $idremitente=$objUser->getId();
    for ($i = 0; $i < $tamanio; $i++)
    {
      list($valorDeclarado,$ciudaddestino, $municipiodestino, $cedula, $nombre, $apellido, $direccion, $telefono, $celular, $observacion) = explode(";", $lineas[$i]);
...
(1,'$ced','$nom','','') ON DUPLICATE KEY UPDATE iddestinatario=LAST_INSERT_ID(iddestinatario), tipo_identificacion_destinatario = 1";
      ...
      if (mysql_query($queryDesti))
      {
        ...
        $rguia = $guia->agregarCC($idCreator);
        if ($rguia == false)
        {
          $qtrans = "ROLLBACK;";
          $sac = $conex->ejecutar($qtrans);
  ...
  if ($rguia === true)
  {
    ...
    <script language="javascript" type="text/javascript">
      ...
      alert(mensaje);
      window.location.href = 'loadUnitaryCC.php';
    </script>
